...
from geneweb_py.core.parser.streaming import should_use_streaming
...
def demo_automatic_mode():
    ...
    small_file = (
        Path(__file__).parent.parent / "tests" / "fixtures" / "simple_family.gw"
    )
    if small_file.exists():
        ...
        print("   Mode utilisé : NORMAL (automatique)")
...
def demo_streaming_mode():
    ...
    if test_file.exists():
        ...
        print("Mode utilisé : STREAMING (forcé)")
        print(f"Temps : {elapsed:.3f}s")
        print(f"Personnes : {len(genealogy.persons)}")
        print(f"Familles : {len(genealogy.families)}")
...
def demo_custom_threshold():
    ...
    GeneWebParser(streaming_threshold_mb=5.0)
    print("   streaming_threshold_mb = 5.0")
    print("   → Économise plus de mémoire, utile pour systèmes contraints")

    print("\n2. Seuil par défaut (10MB) - Équilibré")
    GeneWebParser()
    print("   streaming_threshold_mb = 10.0 (défaut)")
    print("   → Bon compromis temps/mémoire pour la plupart des cas")

    print("\n3. Seuil élevé (50MB) - Streaming moins fréquent")
    GeneWebParser(streaming_threshold_mb=50.0)
    print("   streaming_threshold_mb = 50.0")
    print("   → Privilégie la vitesse sur systèmes avec beaucoup de RAM")
...
def demo_validation_options():
    ...
    parser_validate.parse_file(test_file)
    ...
    parser_no_validate.parse_file(test_file)
    ...
    print("   ⚠️  Utiliser seulement pour fichiers de confiance")
...
def main():
    ...
    print(
        "║"
        + "  DÉMONSTRATION DES OPTIMISATIONS DE PERFORMANCE - geneweb-py".center(78)
        + "║"
    )
    print("║" + " " * 78 + "║")
    print("╚" + "=" * 78 + "╝")
