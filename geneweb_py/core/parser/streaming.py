...
class StreamingLexicalParser:
    ...
    def __init__(self, file_handle: TextIO, filename: Optional[str] = None, buffer_size: int = 8192):
    ...
    def tokenize_lazy(self) -> Iterator[Token]:
        ...
        for line in self.file_handle:
            ...
            if line_stripped.startswith('notes ') or line_stripped.startswith('notes-db'):
                inside_multiline_block = True
                current_block = 'notes' if line_stripped.startswith('notes ') else 'notes-db'
                ...
                if current_block and line_stripped == end_markers.get(current_block, ''):
...
def should_use_streaming(file_path: Union[str, Path], threshold_mb: float = 10.0) -> bool:
...
def estimate_memory_usage(file_path: Union[str, Path]) -> dict:
    ...
    return {
        ...
        'memory_saving_percent': round((1 - streaming_memory_mb / normal_memory_mb) * 100, 1),
