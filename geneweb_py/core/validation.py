...
class ValidationContext:
    ...
    error_collector: GeneWebErrorCollector = field(default_factory=GeneWebErrorCollector)
    strict: bool = False
    ...
    def add_error(self, error: Exception) -> None:
        ...
        if isinstance(error, GeneWebError):
            ...
            self.error_collector.add_error(
                GeneWebValidationError(str(error))
            )
...
def validate_person_basic(person: 'Person', context: Optional[ValidationContext] = None) -> ValidationResult:
    ...
    if not person.last_name or not person.last_name.strip():
        context.add_error(GeneWebValidationError(
...
def validate_person_relationships(person: 'Person', genealogy: 'Genealogy', 
...
def validate_family_basic(family: 'Family', context: Optional[ValidationContext] = None) -> ValidationResult:
...
def validate_family_members(family: 'Family', genealogy: 'Genealogy',
                            context: Optional[ValidationContext] = None) -> ValidationResult:
...
def validate_genealogy_consistency(genealogy: 'Genealogy', 
                                   context: Optional[ValidationContext] = None) -> ValidationResult:
...
def validate_bidirectional_references(genealogy: 'Genealogy',
                                     context: Optional[ValidationContext] = None) -> ValidationResult:
...
def create_partial_person(last_name: str, first_name: str, occurrence: int = 0,
                         error_message: Optional[str] = None) -> 'Person':
...
def create_partial_family(family_id: str, error_message: Optional[str] = None) -> 'Family':
