...
class TitleSchema(BaseModel):
    ...
    start_date: Optional[str] = Field(None, description="Date de début (format ISO ou GeneWeb)")
    end_date: Optional[str] = Field(None, description="Date de fin (format ISO ou GeneWeb)")
...
class PersonBaseSchema(BaseModel):
    ...
    access_level: Union[str, AccessLevel] = Field("default", description="Niveau d'accès")
...
class PersonUpdateSchema(BaseModel):
    ...
    access_level: Optional[Union[str, AccessLevel]] = Field(None, description="Niveau d'accès")
...
class PersonSchema(PersonBaseSchema):
    ...
    events: List[str] = Field(default_factory=list, description="IDs des événements personnels")
...
class PersonSearchSchema(BaseModel):
    ...
    birth_year_from: Optional[int] = Field(None, ge=1000, le=3000, description="Année de naissance minimum")
    birth_year_to: Optional[int] = Field(None, ge=1000, le=3000, description="Année de naissance maximum")
    death_year_from: Optional[int] = Field(None, ge=1000, le=3000, description="Année de décès minimum")
    death_year_to: Optional[int] = Field(None, ge=1000, le=3000, description="Année de décès maximum")
    place: Optional[str] = Field(None, description="Filtrer par lieu")
    access_level: Optional[AccessLevel] = Field(None, description="Filtrer par niveau d'accès")
...
class PersonStatsSchema(BaseModel):
    ...
    by_access_level: Dict[str, int] = Field(..., description="Répartition par niveau d'accès")
    by_century: Dict[str, int] = Field(..., description="Répartition par siècle de naissance")
