...
from ..models.event import (
    EventSchema, PersonalEventCreateSchema, FamilyEventCreateSchema,
    EventUpdateSchema, EventListSchema, EventStatsSchema
)
...
async def create_personal_event(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la création de l'événement: {exc}"
        )
...
async def get_event(
    ...
    try:
        ...
        if event is None:
            raise HTTPException(
                status_code=404,
                detail=f"Événement avec l'ID {event_id} non trouvé"
            )
...
async def list_events(
    ...
    year_from: Optional[int] = Query(None, ge=1000, le=3000, description="Année minimum"),
    ...
    has_witnesses: Optional[bool] = Query(None, description="Filtrer par présence de témoins"),
    has_sources: Optional[bool] = Query(None, description="Filtrer par présence de sources"),
    service: GenealogyService = Depends(get_genealogy_service)
) -> PaginatedResponse:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la recherche des événements: {exc}"
        )
