...
from ..models.family import (
    FamilySchema, FamilyCreateSchema, FamilyUpdateSchema, FamilySearchSchema,
    FamilyListSchema, FamilyStatsSchema
)
...
async def create_family(
    ...
    try:
        ...
        return SuccessResponse(
            ...
            data=family_schema.model_dump()
        )
        
    except Exception as exc:
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la création de la famille: {exc}"
        )
...
async def get_family(
    ...
    if family is None:
        raise HTTPException(
            status_code=404,
            detail=f"Famille avec l'ID '{family_id}' non trouvée"
        )
...
async def list_families(
    ...
    marriage_status: Optional[str] = Query(None, description="Filtrer par statut de mariage"),
    has_children: Optional[bool] = Query(None, description="Filtrer par présence d'enfants"),
    min_children: Optional[int] = Query(None, ge=0, description="Nombre minimum d'enfants"),
    max_children: Optional[int] = Query(None, ge=0, description="Nombre maximum d'enfants"),
    service: GenealogyService = Depends(get_genealogy_service)
) -> PaginatedResponse:
    """
