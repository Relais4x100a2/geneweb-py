...
from ...core.models import (
    Genealogy, Person, Family, Event, PersonalEvent, FamilyEvent,
    Gender, AccessLevel, MarriageStatus, EventType, FamilyEventType
)
...
from ..models.event import PersonalEventCreateSchema, FamilyEventCreateSchema, EventUpdateSchema
...
class GenealogyService:
    ...
    def genealogy(self) -> Genealogy:
        ...
        if self._genealogy is None:
            raise ValueError("Aucune généalogie chargée. Utilisez load_from_file() ou create_empty().")
        return self._genealogy
    ...
    def create_person(self, person_data: PersonCreateSchema) -> Person:
        ...
        if person_id in genealogy.persons:
            ...
            occurrence = 1
            while f"{person_data.surname}_{person_data.first_name}_{occurrence}" in genealogy.persons:
                occurrence += 1
    ...
    def update_person(self, person_id: str, person_data: PersonUpdateSchema) -> Optional[Person]:
        """
    ...
    def delete_person(self, person_id: str) -> bool:
        ...
        for family in genealogy.families.values():
            ...
            if family.wife_id == person_id:
                family.wife_id = None
            family.children = [child for child in family.children if child.person_id != person_id]
    ...
    def search_persons(self, search_params: PersonSearchSchema) -> Tuple[List[Person], int]:
        """
        ...
        if search_params.query:
            ...
            persons = [p for p in persons if (
                query_lower in p.first_name.lower() or
                query_lower in p.last_name.lower() or
                (p.public_name and query_lower in p.public_name.lower())
            )]
        
        if search_params.first_name:
            persons = [p for p in persons if search_params.first_name.lower() in p.first_name.lower()]
        
        if search_params.surname:
            persons = [p for p in persons if search_params.surname.lower() in p.last_name.lower()]
        
        if search_params.sex:
            persons = [p for p in persons if p.gender == search_params.sex]
        
        if search_params.access_level:
            persons = [p for p in persons if p.access_level == search_params.access_level]
    ...
    def update_family(self, family_id: str, family_data: FamilyUpdateSchema) -> Optional[Family]:
        """
    ...
    def search_families(self, search_params: FamilySearchSchema) -> Tuple[List[Family], int]:
        """
        ...
        if search_params.marriage_status:
            families = [f for f in families if f.marriage_status == search_params.marriage_status]
        ...
        if search_params.min_children is not None:
            families = [f for f in families if len(f.children) >= search_params.min_children]
        
        if search_params.max_children is not None:
            families = [f for f in families if len(f.children) <= search_params.max_children]
    ...
    def create_personal_event(self, event_data: PersonalEventCreateSchema) -> PersonalEvent:
        """
    ...
    def update_event(self, event_id: str, event_data: EventUpdateSchema) -> Optional[Event]:
        """
    ...
    def search_events(self, search_params: Dict[str, Any]) -> Tuple[List[Event], int]:
        ...
        if search_params.get('query'):
            ...
            events = [e for e in events if (
                query_lower in (e.place or '').lower() or
                query_lower in (e.reason or '').lower() or
                query_lower in (e.notes or '').lower()
            )]
        ...
        if search_params.get('person_id'):
            events = [e for e in events if hasattr(e, 'person_id') and e.person_id == search_params['person_id']]
        
        if search_params.get('family_id'):
            events = [e for e in events if hasattr(e, 'family_id') and e.family_id == search_params['family_id']]
        
        if search_params.get('place'):
            events = [e for e in events if search_params['place'].lower() in (e.place or '').lower()]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for family in genealogy.families.values():
            # Par statut
            status_key = family.marriage_status.value if family.marriage_status else "married"
        ...
        return {
            ...
            "average_children_per_family": total_children / len(genealogy.families) if genealogy.families else 0,
            "events_by_type": events_by_type,
            "personal_events": personal_events,
            "family_events": family_events,
            "metadata": {
                "source_file": genealogy.metadata.source_file,
                "created": genealogy.metadata.created_date.isoformat() if genealogy.metadata.created_date else None,
                "updated": genealogy.metadata.modified_date.isoformat() if genealogy.metadata.modified_date else None,
