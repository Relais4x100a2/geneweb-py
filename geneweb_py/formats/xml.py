...
class XMLExporter(BaseExporter):
    ...
    def export_to_string(self, genealogy: Genealogy) -> str:
        ...
        try:
            ...
            return ET.tostring(root, encoding=self.encoding, xml_declaration=True).decode(self.encoding)
    ...
    def _serialize_genealogy(self, genealogy: Genealogy) -> ET.Element:
        ...
        events_count = sum(len(p.events) for p in genealogy.persons.values()) + sum(len(f.events) for f in genealogy.families.values())
...
class XMLImporter(BaseImporter):
    ...
    def _deserialize_person(self, elem: ET.Element) -> Optional[Person]:
        ...
        try:
            ...
            if titles_elem is not None:
                ...
                person.titles = [Title(name=title_elem.text) for title_elem in titles_elem.findall("title") if title_elem.text]
    ...
    def _deserialize_family(self, elem: ET.Element) -> Optional[Family]:
        ...
        try:
            ...
            if witnesses_elem is not None:
                family.witnesses = [witness_elem.text for witness_elem in witnesses_elem.findall("witness") if witness_elem.text]
    ...
    def _deserialize_event(self, elem: ET.Element) -> Optional[Event]:
        ...
        try:
            ...
            place = place_elem.text.strip() if place_elem is not None and place_elem.text else None
            ...
            witnesses_elem = elem.find("witnesses")
            if witnesses_elem is not None:
                witnesses = [witness_elem.text for witness_elem in witnesses_elem.findall("witness") if witness_elem.text]
            
            # Notes
            notes = []
            notes_elem = elem.find("notes")
            if notes_elem is not None:
                notes = [note_elem.text for note_elem in notes_elem.findall("note") if note_elem.text]
