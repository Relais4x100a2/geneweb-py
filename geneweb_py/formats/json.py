...
class JSONExporter(BaseExporter):
    ...
    def _serialize_genealogy(self, genealogy: Genealogy) -> Dict[str, Any]:
        ...
        return {
            "metadata": {
                ...
                "created_at": genealogy.metadata.created_date.isoformat() if genealogy.metadata.created_date else None,
                "statistics": {
                    "persons_count": len(genealogy.persons),
                    "families_count": len(genealogy.families),
                    "events_count": sum(len(p.events) for p in genealogy.persons.values()) + 
                                  sum(len(f.events) for f in genealogy.families.values())
                }
            },
            "persons": [self._serialize_person(person) for person in genealogy.persons.values()],
            "families": [self._serialize_family(family) for family in genealogy.families.values()]
        }
    ...
    def _serialize_person(self, person: Person) -> Dict[str, Any]:
        ...
        return {
            ...
            "titles": [{"name": title.name, "title_type": title.title_type, "place": title.place} for title in person.titles],
    ...
    def _serialize_family(self, family: Family) -> Dict[str, Any]:
        ...
        return {
            ...
            "children": [{"person_id": child.person_id, "sex": child.sex.value} for child in family.children],
...
class JSONImporter(BaseImporter):
    ...
    def import_from_string(self, data: str) -> Genealogy:
        ...
        try:
            ...
            if not any(key in json_data for key in ["persons", "families", "metadata"]):
                raise ConversionError("JSON ne contient pas de données de généalogie valides")
    ...
    def _deserialize_person(self, data: Dict[str, Any]) -> Optional[Person]:
        ...
        try:
            ...
            gender = Gender(data.get("gender")) if data.get("gender") else Gender.UNKNOWN
            ...
            raise ConversionError(f"Erreur lors de la désérialisation de la personne : {e}")
    ...
    def _deserialize_family(self, data: Dict[str, Any]) -> Optional[Family]:
        ...
        try:
            ...
            if "children" in data:
                for child_data in data["children"]:
                    child = Child(
                        ...
                        sex=ChildSex(child_data.get("sex", "")) if child_data.get("sex") else ChildSex.UNKNOWN
                    )
            ...
            raise ConversionError(f"Erreur lors de la désérialisation de la famille : {e}")
    ...
    def _deserialize_event(self, data: Dict[str, Any]) -> Optional[Event]:
        ...
        try:
            ...
            raise ConversionError(f"Erreur lors de la désérialisation de l'événement : {e}")
