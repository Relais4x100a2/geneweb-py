...
class TestAPIIntegration:
    ...
    def test_event_creation_workflow(self, client):
        ...
        event_data = {
            ...
            "place": "Paris, France",
            "reason": "Naissance",
            "notes": "Événement de test",
            "witnesses": [],
            "sources": []
        }
        
        response = client.post("/api/v1/events/personal", json=event_data)
        assert response.status_code == 201
        data = response.json()
    ...
    def test_genealogy_stats(self, client):
    ...
    def test_search_functionality(self, client):
        ...
        # Rechercher par nom
        response = client.get("/api/v1/genealogy/search?query=Martin")
        assert response.status_code == 200
        data = response.json()
        ...
        response = client.get("/api/v1/genealogy/search?query=Alice")
        assert response.status_code == 200
        data = response.json()
        assert data["data"]["total_results"] >= 1
    ...
    def test_pagination(self, client):
        ...
        response = client.get("/api/v1/persons?page=1&size=10")
        assert response.status_code == 200
        data = response.json()
        ...
        response = client.get("/api/v1/persons?page=2&size=10")
        assert response.status_code == 200
        data = response.json()
        assert len(data["items"]) == 10
        ...
        response = client.get("/api/v1/persons?page=3&size=10")
        assert response.status_code == 200
        data = response.json()
