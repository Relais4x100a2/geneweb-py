...
def benchmark_parsing(content: str, mode: str = "normal", validate: bool = True) -> Dict[str, Any]:
...
def run_benchmark_suite():
    ...
    for size_name, size_kb in test_sizes:
        ...
        if size_kb >= 1024:
            ...
            try:
                ...
                # Calculer les gains
                if stats_normal:
                    time_ratio = stats_normal['time_seconds'] / stats_streaming['time_seconds']
                    memory_ratio = stats_normal['memory_peak_mb'] / stats_streaming['memory_peak_mb']
    ...
    print(f"{'Taille':<10} {'Mode':<12} {'Temps (s)':<12} {'Mémoire (MB)':<15} {'Personnes':<12}")
    ...
    for result in results:
        ...
        if result['normal']:
            stats = result['normal']
            print(f"{size_name:<10} {'Normal':<12} "
        ...
        if result['streaming']:
            ...
            # Calculer les gains
            if result['normal']:
                time_gain = (1 - stats['time_seconds'] / result['normal']['time_seconds']) * 100
                memory_gain = (1 - stats['memory_peak_mb'] / result['normal']['memory_peak_mb']) * 100
                print(f"{'':<10} {'Gain (%)':<12} "
                      f"{time_gain:<12.1f} "
                      f"{memory_gain:<15.1f}")
...
def benchmark_real_file(file_path: str):
    ...
    if should_use_streaming(file_path):
        ...
        # Gains
        time_gain = (1 - stats_streaming['time_seconds'] / stats_normal['time_seconds']) * 100
        memory_gain = (1 - stats_streaming['memory_peak_mb'] / stats_normal['memory_peak_mb']) * 100
